...
.result-card.fraud { border-left: 3px solid #f85149; }
.result-card.safe  { border-left: 3px solid #3fb950; }
.result-title { font-size: 10px; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 8px; }
.result-title.fraud { color: #f85149; }
.result-title.safe  { color: #3fb950; }
.result-grid { display: grid; grid-template-columns: repeat(4,1fr); gap:10px; margin-top:8px; }
.result-item-label { font-size:8px; color:#484f58; letter-spacing:1px; text-transform:uppercase; margin-bottom:3px; }
.result-item-value { font-size:12px; color:#c9d1d9; }
.summary-bar {
...
.stNumberInput > div > div > input {
    ...
    border: 1px solid #21262d !important; border-radius: 3px !important;
    font-family: 'IBM Plex Mono', monospace !important; font-size: 12px !important;
}
.stNumberInput > div > div > input:focus { border-color: #3fb950 !important; box-shadow: none !important; }
.stSelectbox > div > div {
    background-color: #0d1117 !important; color: #c9d1d9 !important;
    border: 1px solid #21262d !important; border-radius: 3px !important;
    font-family: 'IBM Plex Mono', monospace !important; font-size: 12px !important;
...
.stSelectbox label, .stNumberInput label, .stTextInput label {
    color: #484f58 !important; font-family: 'IBM Plex Mono', monospace !important;
...
# Merge manual alerts at top (most recent first)
if "manual_alerts" in st.session_state and st.session_state.manual_alerts:
    alerts = st.session_state.manual_alerts + alerts
...
def detect_fraud(amount, hour, description=""):
    """
    Rule-based fraud detection engine derived from Krish's XGBoost findings.
    Returns full analysis dict for a given transaction.
    """
    score = 0.0
    attack_type = "Unknown"
    flags = []

    # Rule 1 — Amount signal (strongest per EDA)
    if amount <= 10:
        score += 0.45
        attack_type = "Card Testing"
        flags.append("micro-transaction (sub ₹10)")
    elif amount > 500:
        score += 0.30
        attack_type = "Large Fraud"
        flags.append("high-value transaction")
    elif amount <= 50:
        score += 0.15

    # Rule 2 — Time signal
    if hour in [21, 22, 23, 0, 1, 2, 3]:
        score += 0.30
        flags.append("transaction at high-risk hour")
        if hour in [0, 1, 2, 3] and amount <= 50:
            attack_type = "Bot Attack"
    elif hour in [19, 20]:
        score += 0.15
        flags.append("peak fraud window (9–10 PM)")

    # Rule 3 — Velocity / combined pattern
    if amount <= 10 and hour in [21, 22, 23, 0, 1, 2, 3]:
        score += 0.15  # card testing at night = stronger signal
        attack_type = "Card Testing"

    if amount > 100 and hour in [0, 1, 2, 3]:
        attack_type = "Account Takeover"
        score += 0.10
        flags.append("high amount at unusual hour — ATO pattern")

    if amount > 200 and amount <= 500 and hour in range(9, 18):
        attack_type = "Velocity Abuse"
        flags.append("mid-range daytime velocity pattern")

    # Cap score
    score = min(round(score, 3), 0.99)

    # Fraud threshold — 0.40 and above = fraud
    is_fraud = score >= 0.40

    if not is_fraud:
        attack_type = "—"
        flags = ["No significant fraud indicators detected"]
    ...
    # V14 estimate (proxy — derived from amount+hour)
    v14_est = round(-0.5 - (score * 3.5), 3) if is_fraud else round(random.uniform(-0.3, 0.3), 3)
...
if "rows" not in st.session_state:
    st.session_state.rows = [{"amount": None, "hour": None, "desc": ""}]
...
with btn_col:
    if st.button("＋  ANALYSE TRANSACTION", key="open_form"):
...
<div class="form-header">
    <span>▶ TRANSACTION ANALYSER — SUBMIT FOR FRAUD DETECTION</span>
    <span style="font-size:9px; color:#8b949e">Fill in transaction details · FraudSOC will analyse and classify</span>
</div>
""", unsafe_allow_html=True)

    st.markdown("""
<div style="background:#0d1117; border:1px solid #21262d; border-top:none; padding:14px 20px 4px;">
<div style="display:grid; grid-template-columns:40px 1fr 1fr 2fr 50px; gap:10px;
            font-family:'IBM Plex Mono',monospace; font-size:9px; color:#484f58;
            letter-spacing:2px; text-transform:uppercase;
            padding-bottom:8px; border-bottom:1px solid #21262d; margin-bottom:8px;">
    <div>#</div><div>Amount (₹)</div><div>Time</div><div>Description (optional)</div><div></div>
</div>
</div>
""", unsafe_allow_html=True)
    ...
    for i, row in enumerate(st.session_state.rows):
        cols = st.columns([0.3, 1, 1, 2, 0.3])
        with cols[0]:
            st.markdown(f"<div style='font-family:IBM Plex Mono;font-size:10px;color:#484f58;padding-top:32px;text-align:center;'>{i+1:02d}</div>", unsafe_allow_html=True)
        with cols[1]:
            amt = st.number_input(
                "AMOUNT (₹)",
                min_value=0.01, max_value=99999.00,
                value=float(row["amount"]) if row["amount"] else None,
                step=0.01, format="%.2f",
                key=f"amt_{i}",
                placeholder="e.g. 4.50"
            )
            st.session_state.rows[i]["amount"] = amt
        with cols[2]:
            hour_options = [f"{h:02d}:00 — {'AM' if h < 12 else 'PM'} (Hour {h})" for h in range(24)]
            hour_labels  = ["Select time..."] + hour_options
            sel = st.selectbox(
                "TIME",
                options=hour_labels,
                index=0 if row["hour"] is None else row["hour"] + 1,
                key=f"hr_{i}"
            )
            st.session_state.rows[i]["hour"] = hour_labels.index(sel) - 1 if sel != "Select time..." else None
        with cols[3]:
            desc = st.text_input(
                ...
                max_chars=100,
                key=f"desc_{i}",
                placeholder="e.g. I did not make this payment"
            )
            st.session_state.rows[i]["desc"] = desc
        with cols[4]:
    ...
    # Add row + Analyse buttons
    act_col1, act_col2, act_col3 = st.columns([1, 1, 6])
    with act_col1:
        if st.button("＋  ADD ROW", key="add_row"):
            st.session_state.rows.append({"amount": None, "hour": None, "desc": ""})
            st.rerun()
    with act_col2:
        # Validate — all rows need amount and hour
        valid_rows = [r for r in st.session_state.rows
                      if r["amount"] is not None and r["amount"] > 0
                      and r["hour"] is not None and r["hour"] >= 0]
        analyse_disabled = len(valid_rows) == 0

        if st.button(
            "⚡  ANALYSE" + (" (%d)" % len(valid_rows) if valid_rows else " (fill fields)"),
            key="analyse_btn",
            disabled=analyse_disabled
        ):
            results = []
            for r in valid_rows:
                result = detect_fraud(r["amount"], r["hour"], r["desc"])
                result["alert_id"] = f"MAN-{random.randint(1000,9999)}"
                result["time"] = f"{r['hour']:02d}:00"
                results.append(result)

            # Add fraud ones to alert queue
            new_alerts = []
            for res in results:
                if res["is_fraud"]:
                    new_alerts.append({
                        ...
                        "amount_raw": res["amount"],
                        "attack_type":res["attack_type"],
                        "severity":   res["severity"],
                        "status":     "OPEN",
                        "v14_score":  res.get("v14_est", round(random.uniform(0.6,0.99),3)),
                        "fraud_prob": res["fraud_prob"],
                        "manual":     True,
                    })

            # Prepend to alert list
            st.session_state.manual_alerts = new_alerts + st.session_state.manual_alerts
            st.session_state.results = results
            st.session_state.show_results = True
            st.rerun()

    # Validation hint
    if len(valid_rows) < len(st.session_state.rows):
        incomplete = len(st.session_state.rows) - len(valid_rows)
        st.markdown(f"<div style='font-family:IBM Plex Mono;font-size:9px;color:#d29922;padding:4px 0;'>"
                    f"⚠ {incomplete} row(s) incomplete — fill Amount and Time to include in analysis</div>",
                    unsafe_allow_html=True)
...
<div class="summary-bar">
    ...
    <span style="color:#f85149">⬤ {fraud_count} FRAUD DETECTED</span>
    <span style="color:#3fb950">⬤ {safe_count} SAFE</span>
    <span style="color:#8b949e">{len(results)} TRANSACTIONS ANALYSED</span>
    <span style="color:#484f58; margin-left:auto">
        {'Fraud alerts added to queue below ↓' if fraud_count > 0 else 'No fraud — nothing added to queue'}
    </span>
</div>
""", unsafe_allow_html=True)

    for res in results:
        fraud_class = "fraud" if res["is_fraud"] else "safe"
        verdict     = "⚠ FRAUD DETECTED" if res["is_fraud"] else "✓ TRANSACTION SAFE"
        prob_color  = "#f85149" if res["is_fraud"] else "#3fb950"
        st.markdown(f"""
<div class="result-card {fraud_class}">
    <div class="result-title {fraud_class}">{verdict} — ₹{res['amount']:.2f} at {res['time']}</div>
    <div class="result-grid">
        <div>
            <div class="result-item-label">Fraud Probability</div>
            <div class="result-item-value" style="color:{prob_color}; font-weight:600">{res['fraud_prob']:.1%}</div>
        </div>
        <div>
            <div class="result-item-label">Attack Type</div>
            <div class="result-item-value">{res['attack_type']}</div>
        </div>
        <div>
            <div class="result-item-label">Severity</div>
            <div class="result-item-value">{res['severity']}</div>
        </div>
        <div>
            <div class="result-item-label">MITRE ATT&CK</div>
            <div class="result-item-value" style="color:#bc8cff">{res['mitre']}</div>
        </div>
    </div>
    <div style="margin-top:8px; font-size:9px; color:#484f58; font-family:'IBM Plex Mono',monospace;">
        FLAGS: {' · '.join(res['flags'])}
    </div>
</div>
""", unsafe_allow_html=True)

    if st.button("✕  CLEAR RESULTS & CLOSE", key="clear_results"):
        st.session_state.show_results = False
        st.session_state.show_form = False
        st.session_state.rows = [{"amount": None, "hour": None, "desc": ""}]
        st.rerun()
...
<div class="kpi-row">
    ...
    <div class="kpi-card orange">
        ...
        <div class="kpi-value">{sev_counts['CRITICAL'] + sev_counts['HIGH'] + len([a for a in st.session_state.get('manual_alerts',[]) if a.get('severity') in ['CRITICAL','HIGH']])}</div>
...
for a in filtered[:20]:
    ...
    manual_tag = ' <span style="font-size:8px;background:rgba(63,185,80,0.1);border:1px solid rgba(63,185,80,0.2);color:#3fb950;padding:1px 5px;border-radius:2px;">MANUAL</span>' if a.get("manual") else ""
    table_html += f"""
<div class="alert-row {sev_lower}">
    <div style="color:#58a6ff">{a['alert_id']}{manual_tag}</div>
    <div>{a['time']}</div>
...
# Compute most active attack type from current alert queue
from collections import Counter
attack_counts = Counter(a["attack_type"] for a in alerts)
top_attack = attack_counts.most_common(1)[0][0] if attack_counts else "Card Testing"

# Playbook data keyed by attack type
PLAYBOOKS = {
    "Card Testing": [
        ("Immediately block IP range / device fingerprint associated with micro-transaction burst", "ACTION: Network / WAF Rule"),
        ("Flag all transactions under ₹9.25 from same source within 60-minute window", "ACTION: Rule Engine Update"),
        ("Escalate to Tier-2 analyst if V14 score exceeds 0.80 on any flagged transaction", "ACTION: Human Escalation"),
        ("Issue temporary hold on affected card/UPI handle — notify user via SMS", "ACTION: Account Protection"),
        ("Document incident, update XGBoost training queue with confirmed fraud label", "ACTION: Model Feedback Loop"),
    ],
    "Bot Attack": [
        ("Identify and rate-limit device fingerprint / IP subnet generating high-frequency requests", "ACTION: WAF / Rate Limiting"),
        ("Enable CAPTCHA challenge on payment API endpoints flagged by velocity rules", "ACTION: Anti-Bot Defense"),
        ("Block all transactions from flagged source for 24 hours — escalate to SOC Tier-2", "ACTION: Human Escalation"),
        ("Analyse transaction timing intervals — sub-second gaps confirm bot origin", "ACTION: Forensic Analysis"),
        ("File incident report and update velocity detection thresholds in rule engine", "ACTION: Model Feedback Loop"),
    ],
    "ATO": [
        ("Immediately freeze the compromised UPI handle / account pending verification", "ACTION: Account Protection"),
        ("Force password reset and step-up MFA for affected user — notify via registered mobile", "ACTION: Identity Verification"),
        ("Review last 72 hours of login events for credential stuffing indicators", "ACTION: Threat Hunting"),
        ("Escalate to Fraud Response Team — initiate customer contact for confirmation", "ACTION: Human Escalation"),
        ("File STR with FIU-IND if transaction value exceeds ₹10L threshold per PMLA", "ACTION: Regulatory Compliance"),
    ],
    "Large Fraud": [
        ("Immediately reverse or hold the transaction — contact acquiring bank within SLA window", "ACTION: Transaction Hold"),
        ("Freeze all outbound transfers from affected account for 48 hours", "ACTION: Account Protection"),
        ("Escalate directly to Tier-3 / Fraud Response Team — this is a P1 incident", "ACTION: Human Escalation"),
        ("Preserve all logs, device fingerprints and session data for forensic investigation", "ACTION: Evidence Preservation"),
        ("File Suspicious Transaction Report (STR) with FIU-IND — mandatory under PMLA", "ACTION: Regulatory Compliance"),
    ],
    "Velocity Abuse": [
        ("Apply per-account rate limiting — cap transactions at 5 per hour from same source", "ACTION: Rule Engine Update"),
        ("Flag the merchant / payee as high-risk and trigger enhanced monitoring", "ACTION: Threat Intelligence"),
        ("Escalate to Tier-2 if velocity pattern persists beyond 30 minutes after rule update", "ACTION: Human Escalation"),
        ("Analyse payee network for money mule patterns — check linked accounts", "ACTION: Network Analysis"),
        ("Update XGBoost feature pipeline with velocity feature — retrain if pattern is novel", "ACTION: Model Feedback Loop"),
    ],
}

steps = PLAYBOOKS.get(top_attack, PLAYBOOKS["Card Testing"])
steps_html = ""
for idx, (text, label) in enumerate(steps, 1):
    steps_html += f"""
    <div class="step">
...
with col_play:
    playbook_html = (
        ...
        + f'▶ INCIDENT RESPONSE PLAYBOOK — {top_attack.upper()} ATTACK'
        + f'<span style="font-size:8px; color:#8b949e; margin-left:10px;">MOST ACTIVE · {attack_counts[top_attack]} ALERTS</span>'
        + '</div>'
        + steps_html
        + '</div>'
    )
...
CHIPS = [
    "Why fraud peaks at 9 PM?",
    "Should we block sub ₹10 transactions?",
    "Why is V14 so important?",
    "XGBoost vs Random Forest — which to deploy?",
...
# Chip buttons — 3 per row for clean alignment
...
# If a chip was clicked, process it directly — don't use text_input value
query_to_run = ""
...
# Manual text input takes priority if typed
if query:
    query_to_run = query
...
if query_to_run:
    if groq_ok:
        with st.spinner("Analyzing..."):
            answer = ask_analyst(query_to_run, st.session_state.chat_history)
        st.session_state.chat_history.append((query_to_run, answer))
    else:
        st.session_state.chat_history.append((query_to_run,
